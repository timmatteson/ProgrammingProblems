...
Task
We will need two functions. The first one will receive a decimal number and return a string withd the factorial representation.
...
factDigit = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ "
...
def dec2FactString(nb):
    ...
    for i in range(len(factDigit)):
        if math.factorial(i) > nb:
            start = i - 1
            break

    while start > -1:
        for i in range(len(factDigit)):
            if math.factorial(start) * i > accum:
                result += factDigit[i - 1]
                accum -= math.factorial(start) * (i - 1)
                break
...
def factString2Dec(string):
    ...
    for i in range(len(string)):
        result += math.factorial(i) * factDigit.index(string[::-1][i])
